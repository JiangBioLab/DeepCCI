...
def pretarin_cluster(n_clusters):
...
def train_cluster(dataset):
    ...
    if Auto:
        if z.shape[0] < 2000:
            ...
            resolution = 0.5
        n_clusters = int(args.n_clusters*resolution) if int(args.n_clusters*resolution)>=3 else 2
    else:
...
if __name__ == "__main__":
    ...
    if pretain:
        premodel_i = pretarin_cluster(n_clusters)
        #print(premodel_i)
        #pretrain_path
        args.pretrain_path = './dataset/'+args.name+'/model/'+str(premodel_i)+'.pkl'
    else:
        #pretain_model
        args.pretrain_path = './pretain_model/'+args.name+'/'+args.name+'.pkl'

    dataset = load_data(args.name)
    train_cluster(dataset)